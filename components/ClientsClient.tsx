...
import type { Client, ClientStatus, Engagement } from '@/lib/types'

type CWE = Client & { engagements: Engagement[] }
const STATUSES: ClientStatus[] = ['Active', 'On Hold', 'Completed']
...
export default function ClientsClient({ initialClients }: { initialClients: CWE[] }) {
  ...
  const [selected, setSelected] = useState<CWE | null>(null)
  ...
  function openEdit(c: Client, e: React.MouseEvent) {
    e.stopPropagation()
    setEditing(c); setForm({ name: c.name, contact_name: c.contact_name, phone: c.phone, email: c.email, status: c.status }); setShowForm(true)
  }

  async function save() {
    setSaving(true)
    if (editing) {
      const { data } = await supabase.from('clients').update(form).eq('id', editing.id).select('*, engagements(*)').single()
      if (data) setClients(p => p.map(c => c.id === editing.id ? data : c))
    } else {
      const { data } = await supabase.from('clients').insert(form).select('*, engagements(*)').single()
  ...
  async function del(c: Client) {
    ...
    setClients(p => p.filter(x => x.id !== c.id)); setDelTarget(null)
    if (selected?.id === c.id) setSelected(null)
  }

  function onEngChange(id: string, eng: Engagement[]) {
    setClients(p => p.map(c => c.id === id ? { ...c, engagements: eng } : c))
    if (selected?.id === id) setSelected(p => p ? { ...p, engagements: eng } : p)
  }

  const ToggleRow = ({ opts, val, set }: { opts: string[]; val: string; set: (v: string) => void }) => (
    <div className="flex gap-2">
      {opts.map(o => (
        <Button key={o} type="button" variant={val === o ? 'default' : 'outline'} size="sm"
          className="flex-1 text-xs" onClick={() => set(o)}>
          {o}
        </Button>
      ))}
    </div>
    ...
    <div className="max-w-lg mx-auto px-4 pt-8 pb-4">
      ...
      <div className="flex gap-1.5 mb-4 overflow-x-auto pb-0.5">
        {FILTER.map(f => (
          <Button key={f} size="sm" variant={filter === f ? 'default' : 'outline'}
            className="shrink-0 text-xs h-7 px-2.5" onClick={() => setFilter(f)}>
            {f}
          </Button>
        ))}
      </div>
      ...
      <Card>
        <CardContent className="p-0">
          ...
          {filtered.map((c, i) => (
            <div key={c.id}>
              {i > 0 && <Separator />}
              <div onClick={() => setSelected(c)} className="flex items-center gap-3 px-4 py-3.5 hover:bg-muted/40 cursor-pointer transition-colors">
      ...
      {showForm && (
        <Modal title={editing ? 'Edit Client' : 'New Client'} onClose={() => setShowForm(false)}>
          <div className="space-y-3">
            {[['name','Company name','Acme Inc.'],['contact_name','Contact name','Jane Smith'],['phone','Phone','+1 234 567'],['email','Email','jane@acme.com']].map(([f, l, p]) => (
              <div key={f} className="space-y-1.5">
                <Label>{l}</Label>
                <Input type={f === 'email' ? 'email' : f === 'phone' ? 'tel' : 'text'}
                  value={(form as any)[f]} onChange={e => setForm(x => ({ ...x, [f]: e.target.value }))} placeholder={p} />
              </div>
            ))}
            <div className="space-y-1.5">
              <Label>Status</Label>
              <ToggleRow opts={STATUSES} val={form.status} set={v => setForm(p => ({ ...p, status: v as ClientStatus }))} />
            </div>
      ...
      {delTarget && (
        <Modal title="Delete client?" onClose={() => setDelTarget(null)}>
          <p className="text-sm text-muted-foreground mb-4">Delete <strong className="text-foreground">{delTarget.name}</strong> and all their engagements?</p>
          <div className="flex gap-2">
      ...
      {selected && (
        <Modal title={selected.name} onClose={() => setSelected(null)}>
          <EngagementsPanel client={selected} engagements={selected.engagements} onChange={e => onEngChange(selected.id, e)} />
        </Modal>
      )}
    </div>
  )
